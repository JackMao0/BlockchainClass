...
//1.读取账户bnb个数
const getBNBBalance = async (address,web3) => {
...
//2.获得代币数量
const getTokenBalance = async (tokenaddress, address,web3) => {
    //创建代币的智能合约函数
...
const  getTokenToTokenRate = async (tokenToTokenAddress,tokenAddress,wbnbaddress,web3)=>{
    ...
    let safemoon = await getTokenBalance(safemoonaddress, safemoonpair,web3);
    let wbnb = await getTokenBalance(wbnbaddress, safemoonpair,web3);
...
//2.获取input内容
function swapEthToTokensInput(wbnbadddress, toaddress, tokenamountIn, amountOut, tokenaddress,input_type,path,web3, tokendecimals = 18,) {
...
function swapTokensToEthInput(payContractAddress, fromAddress, payNum, getNum, getContractAddress,input_type,path,web3, decimals = 18,) {
...
const signTransaction = async (fromAddress, toAddress, input, nonceNum, privKey, gasPrice, nbnb, gaslimit,web3,chainId) => {
    var serializedTx = getEthRawTx(fromAddress, toAddress, input, nonceNum, privKey, gasPrice, nbnb, gaslimit,web3,chainId)
...
//5.这里是将交易用私钥签名部分
function getEthRawTx(fromAddress, toAddress, input, nonceNum, privKey, gasPrice, nbnb, gaslimit,web3,chainId) {
...
// swap系列相关方法最后的方法
const swapEthToTokens = async (prikeystring,tokenaddress,wbnbaddress,addresspancake,input_type,path,web3,chainId) => {
    ...
    var gaslimit = 420000
    var input = swapEthToTokensInput(wbnbaddress, fromAddress, nbnb, ntoken, tokenaddress,input_type,path,web3, decimals)
    let reslut = await signTransaction(fromAddress, toAddress, input, nonceCnt, priKey, gasPrice, nbnb, gaslimit,web3,chainId)
    if (reslut) {
        console.log("交易成功")
    }
    else {
...
// swap系列相关方法最后的方法
const swapTokensToEth = async (priKey,payContractAddress,getContractAddress,pancakeAddress,input_type,path,web3,chainId) => {
    ...
    //设置 gaslimit 为 420000
    var gaslimit = 420000
    var input = swapTokensToEthInput(payContractAddress, fromAddress, payNum, getNum, getContractAddress,input_type,path,web3, decimals)
    let reslut = await signTransaction(fromAddress, toAddress, input, nonceCnt, priKey, gasPrice, 0, gaslimit,web3,chainId)
    if (reslut) {
        console.log("交易成功")
    }
    else {
...
const tokenToToken = async(parameter_map,input_type,path,web3,chainId)=>{
    ...
    let reslut = await signTransaction(fromAddress, pancakeaddress, input, nonceCnt, priKey, gasPrice, 0, gaslimit,web3,chainId)
    if (reslut) {
        console.log("交易成功")
    }
    else {
...
const send = async (prikeystr,toAddress,web3,chainId) => {
...
const transferByAbi = async (prikeystr,payContractAddress,getAccountAddress,payNum,input_type,web3,chainId) => {
    var prikeystring = prikeystr.replace("0x", "")
    //这里的privKey 是私钥
    var priKey = getPriKey(prikeystring);
    //获得自己的地址
    ...
    //获取input
    var input = web3.eth.abi.encodeFunctionCall(input_type,[getAccountAddress,amountInNum]);
    ...
    console.log("发送地址是：" + fromAddress)
    var nonceCnt = await web3.eth.getTransactionCount(fromAddress);
    let reslut = await signTransaction(fromAddress, payContractAddress, input, nonceCnt, priKey, gasPrice, 0, gaslimit,web3,chainId)
    if (reslut) {
        console.log("交易成功")
    }
    else {
...
const approveByContractAbi = async (prikeystr,payContractAddress,getAccountAddress,payNum,input_type,web3,chainId) => {
    var prikeystring = prikeystr.replace("0x", "")
    //这里的privKey 是私钥
    var priKey = getPriKey(prikeystring);
    //获得自己的地址
...
module.exports = {
    ...
    "approveByContractAbi":approveByContractAbi
}