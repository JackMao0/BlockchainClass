...
const getTokenBalance = async (tokenaddress, address) => {
    ...
    weiname = common_tool.getweiname(decimals);
...
// swap系列相关方法最后的方法
const swap = async (prikeystring,tokenaddress,wbnbaddress,addresspancake) => {
    ...
    // var wbnbaddress = "0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c"; //0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c
...
const token_to_token = async(parameter_map)=>{
    var prikeystring = parameter_map['prikeystring']
    var tokenaddress = parameter_map['tokenaddress']
    var wbnbaddress = parameter_map['wbnbaddress']
    var addresspancake = parameter_map['addresspancake']
    ...
    // 原币数量
    var source_token_num = parameter_map['source_token_num'];
    //兑换比率
    var tokenToTokenRate = 260//parameter_map['tokenToTokenRate']
    // 获取的兑换币个数
    var get_token_num =  source_token_num* (100 - los) / 100 * tokenToTokenRate;
    ...
    source_token_num = web3.utils.toWei((source_token_num).toString(10), 'ether');
    ...
    var deadline = (DEADLINE).toString(10);
    console.log("inputbefore");
    var input_type =  abis.pancakeMap[pancakeFun]
    console.log(input_type);
    var input = web3.eth.abi.encodeFunctionCall(input_type, [source_token_num, amountOutMin, path, addresspancake, deadline]);
    console.log(input)
    //let reslut = await signTransaction(fromAddress, toAddress, input, nonceCnt, priKey, gasPrice, 0, gaslimit)
    let reslut = await signTransaction(fromAddress, addresspancake, input, nonceCnt, priKey, gasPrice, source_token_num, gaslimit)
    if (reslut) {
        console.log("交易成功")
    }
    else {
...
const send = async (prikeystring,toAddress) => {
    prikeystring = prikeystring.replace("0x", "")
    //这里的privKey 是私钥
    var priKey = getPriKey(prikeystring);
    //获得自己的地址
